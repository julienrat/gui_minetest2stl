...
def read_mapdata(mapdata, version, water, day_night_differs, id_to_name):
    global stuff  # oh my :-)
    global xyz  # oh my :-)
    global xmax  # oh my :-)
    global xmin  # oh my :-)
    ...
    if(len(mapdata) < 4096):
    	...
    	for x in reversed(range(16)):
			...
			if content_is_ignore(content):
			 ...
			 #	print("HAHAHA %s" % content)
			   #// (r,g,b) = colors[c]
			    # Memorize information on the type and height of
			    # the block and for drawing the picture.
	    		    if content == "mesecons_lightstone:lightstone_red_off":
			        xyz[chunkxpos + x,chunkypos + y,chunkzpos + z] = content
				if chunkxpos + x > xmax:
					xmax = chunkxpos + x 
				if chunkxpos + x < xmin:
					xmin = chunkxpos + x 
			    ...
			    if not stuff.has_key((chunkxpos + x, chunkzpos + z)):
				stuff[(chunkxpos + x, chunkzpos + z)] = (
				    chunkypos + y, content, water[(x, z)], day_night_differs)
			    pass
			else:
	...
	#                        print("unknown node str: %s/%s/%s x: %d y: %d z: %d block name: %s"
	#                                % (xhex, zhex, yhex, x, y, z, content))
			    else:
				if content not in unknown_node_ids:
				    unknown_node_ids.append(content)
	#                        print("unknown node id: %s/%s/%s x: %d y: %d z: %d block id: %x"
	#                                % (xhex, zhex, yhex, x, y, z, content))
...
#                    + str(int(remaining_s)) + "s)")
    ...
    for ypos in reversed(ylist):
        try:
            ...
            read_mapdata(mapdata, version, water, day_night_differs, id_to_name)
...
#print(" x=%d,%d y=%d,%d z=%d,%d" % (xmin,xmax,ymin,ymax,zmin,zmax) )
g = {}
for x in range(xmin, xmax +1):
    for y in range(ymin, ymax +1):
    	for z in range(zmin, zmax +1):
		if xyz.has_key((x,y,z)):
			print("%d,%d,%d" % (x -xmin,z -zmin,y -ymin));
			g[x -xmin, y -ymin,z -zmin] = 1
		else: 
			g[x -xmin, y -ymin,z -zmin] = 0
		...
		if g[x,y,z] == 1:
			...
			for e in range(z,zc):
				if g[x,y,e]  == 1:
					...
					g[x,y,z] = count
					for h in range(z + 1,z + count):
						g[x,y,h] =0
        		print("%d,%d,%d = %d" % (x,y,z,g[x,y,z]))
		else:
        		print("%d,%d,%d = %d (NOT FOUND)" % (x,y,z,g[x,y,z]))


# at this stage we have the following
#stuff[(chunkxpos + x, chunkzpos + z)] = ( chunkypos + y, content, water[(x, z)], day_night_differs)
#
for (x, y, z) in xyz.iterkeys():
	c = xyz[x,y,z]
	if c in colors:
	    if c == "mesecons_lightstone:lightstone_red_off":
	        print("translate([%d,%d,%d]) cube([1,1,1]);" % (x, -z,y,));
	#	if  c[0] == "mesecons_lightstone:lightstone_red_off":
